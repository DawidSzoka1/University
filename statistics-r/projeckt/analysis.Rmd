...
wniosek_mean <- if(pval > 0.05){
    paste0("Nie ma podstaw do odrzucenia $H_0$ na poziomie istotności 
    $\\alpha = 0.05$ (p = ", pval, " > 0.05).  
Oznacza to, że nie istnieje **statystycznie istotna różnica** w średnich kosztach paliwa pomiędzy samochodami z napędem na tylne i przednie koła.")
  } else {
    paste0("Odrzucamy $H_0$ na rzecz hipotezy alternatywnej $H_1$ na poziomie istotności $\\alpha = 0.05$ (p = ", pval, " < 0.05).  
Oznacza to, że istnieje **statystycznie istotna różnica** w średnich kosztach paliwa pomiędzy samochodami z napędem na tylne i przednie koła.")
  }
```
`r wniosek_mean`

#### Przetestowanie hipotez równości `fuelCost08` dla wariacji 
...
wniosek_var <- if (pval_var > 0.05) {
  paste0("Nie ma podstaw do odrzucenia $H_0$ na poziomie istotności $\\alpha = 0.05$ 
  (p = ", round(pval_var, 4), " > 0.05).  
Oznacza to, że nie istnieje **statystycznie istotna różnica** w wariancjach kosztów paliwa pomiędzy samochodami z napędem na tylne i przednie koła.")
  ...
  paste0("Odrzucamy $H_0$ na rzecz hipotezy alternatywnej $H_1$ na poziomie istotności 
  $\\alpha = 0.05$ (p = ", round(pval_var, 4), " < 0.05).  
...
Ponieważ wartość p = `r signif(wilcox_result$p.value, 5)` jest mniejsza niż 0,05, odrzucamy hipoteze zerową $H_0$ na rzecz hipotezy alternatywnej $H_1$ na poziomie istotności $\alpha = 0.05$. Oznacza to, że istnieje statystycznie istotna różnica w medianie rocznego zużycia paliwa między pojazdami z napędem na tylne i przednie koła.
