...
```
## Pierwsza część
Na samym starcie wyznaczymy dla każdej zmiennej odpowiednie parametry liczbowe i pozycjne co zostanie przedstawione w postaci jednej tabeli.
...
jak i hipotezami po uwczesnym sprawdzeniu stosowanlości testów.

**Średnia i wariancja: **
Jako zmienną z rokładem normalnym będzie `youSaveSpend` oczywiście sprawdzimy czy tak jest.
Do sprawdzenia tego użyjemy `shapiro.test` z próbka o wielkości 40.

**Mediana:**
Zbadany dla zmiennej `city08` po uwczesnym odrzuceniu hipotezy o rozkładznie normalnym.
Do sprawdzenia czy można odrzucić hipoteze użyjemy `shapiro.test` z próbka o wielkości 80.

**Frakcja:**
Weźmiemy zmienną `startStop` dla której sprawdzimy stosowalność jak i jakiego testu należy użyć.
Z próbka o wielkości 100.


Wszystkie próbi są losowe z ustawionym ziarnem na 20250322. Wszystkie zmienne zostaną odpowiednio przygotowane do testowania czyli pominiecie wartości NA i losowa próbka o właściwej wielkości
...
```
## Wniosek dotyczący stosowalności przedziałów ufności
...
## Mediania sprawdzenie stosowalności dla `city08`
```{r echo=FALSE}
...
```
Dla zmiennej `city08` przeprowadzono test normalności Shapiro-Wilks
...
## Czwarta część.
**Wizualizacja danych (na podstawie próbki 150 rekordów)**
...
## Część piąta. Analiza regresji liniowej.
Celem tej części projektu była analiza zależności pomiędzy pojemnością silnika (displ) a zużyciem paliwa w cyklu miejskim (city08). Obie zmienne mają charakter ilościowy, co uzasadnia zastosowanie modelu regresji liniowej.

**Wizualizacja regresji:**

W celu lepszego zobrazowania zależności, do analizy dołączono wykresy:

- rozrzutu reszt który pomaga ocenić, czy model regresji jest dobrze dopasowany do danych.
- Q-Q plot by sprawdzić czy dane mają rozkład zbliżony do normalnego  
- wykres zależności tych zmiennych od siebie.
