...
opisz_liczbowa <- function(x) {
  ...
  tibble(
    ...
    A = e1071::skewness(x, na.rm = TRUE),
    kurtosis = kurtosis_x,
    K = kurtosis_x - 3,
    G = gini_coef(x),
    min = x_min,
    max = x_max,
    Mo = as.numeric(names(sort(table(x), decreasing = TRUE)[1])),
    Me = median_x,
    Q1 = Q1_x,
    Q3 = Q3_x,
    D1 = D1_x,
    ...
    IQR = IQR_x,
    V_Q = Q_x / median_x,
    V_Q1Q3 = (Q3_x - Q1_x) / (Q3_x + Q1_x),
    T_Q = paste0("(", round(median_x - Q_x, 2), " - ", round(median_x + Q_x, 2), ")"),
    W_sQ = (Q3_x - median_x) - (median_x - Q1_x),
    A_Q = (Q3_x + Q1_x - 2 * median_x) / (2 * Q_x),
    W_x = (D9_x - D1_x) / (Q3_x - Q1_x)
  )
}
...
- wykres rozrzutu reszt, który pomaga ocenić, czy model regresji jest dobrze dopasowany do danych,

- wykres Q-Q plot, by sprawdzić, czy reszty mają rozkład zbliżony do normalnego,

- wykres zależności badanych zmiennych od siebie.
...
- GVLMA:
  ...
  - Heteroskedastyczność nie występuje (p = 0.186).
- Durbin-Watson (p = 0.44): brak autokorelacji.

Podsumowanie:

- Istnieje silna, ujemna zależność pomiędzy pojemnością silnika a zużyciem paliwa w mieście.
...
### Wyznaczanie zbioru zmiennych objaśniających
```{r}
model_end <- step(lm(city08 ~ displ, data = data_clean), direction = "both")
summary(model_end)
```
